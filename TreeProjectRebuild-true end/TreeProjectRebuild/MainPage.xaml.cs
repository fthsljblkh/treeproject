...
namespace TreeProjectRebuild
{
    ...
    public sealed partial class MainPage : Page
    {
        ...
        internal static bool selectMode = false;
        ...
        void MainPage_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            confirmButton = new Button { Content = "Confirm", FontSize = 36 };
            Canvas.SetLeft(confirmButton, canvas.ActualWidth - 180);
            Canvas.SetTop(confirmButton, canvas.ActualHeight / 2);
        ...
        void confirmButton_Tapped(object sender, TappedRoutedEventArgs e)
        {
            if(RootView.ctrlSelected.Count > 0){
                RootView.trace.Push(currentViews);

                canvas.Children.Clear();
                double centerX = BranchView.CalcRadius(0, RootView.ctrlSelected);
                double centerY = RootView.canvas.ActualHeight / 4 * 3;

                for (int i = 0; i < RootView.ctrlSelected.Count; ++i)
                {
                    double r = BranchView.CalcRadius(i, RootView.ctrlSelected);
                    RootView.SetElli(centerX, centerY, r, Colors.Black, Colors.Gray, RootView.ctrlSelected[i]);
                    if (RootView.ctrlSelected[i].rootModel.Level == GeneralLevel.Decade_Country)
                        RootView.ctrlSelected[i].show_notTangent(GeneralLevel.Year_Province, 0.1, 1.2, 4, 2);
                    else if (RootView.ctrlSelected[i].rootModel.Level == GeneralLevel.Year_Province)
                        RootView.ctrlSelected[i].show_notTangent(GeneralLevel.Season_City, 0.2, 1.2, 6, 4);
                    else
                        RootView.ctrlSelected[i].show_notTangent(GeneralLevel.Season_City, 0.3, 1.3, 8, 6);
                    centerX += 2 * r;
                }

                canvas.Children.Add(returnButton);
                RootView.setCurrentView(new BranchView(RootView.ctrlSelected));

                RootView.ctrlSelected.Clear();
            }
        }
        ...
        {
            if (e.Key == Windows.System.VirtualKey.Control && (!selectMode))
                selectMode = true;
        }
